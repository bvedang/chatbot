...
export const searchTool = ({ streamingData, fullResponse }: ToolProps) =>
  tool({
    ...
    execute: async ({
      ...
      console.log("searchdepth", search_depth);
      let hasError = false;
      ...
      const effectiveSearchDepth = search_depth || "basic";

      console.log(
        `Using search API: SearXNG, Search Depth: ${effectiveSearchDepth}`
      );

      try {
        if (effectiveSearchDepth === "advanced") {
          searchResult = await advancedSearchXNGSearch(
            filledQuery,
            max_results,
...
async function searxngSearch(
  ...
  try {
    ...
    const filteredResults = data.results.filter((result) => {
      ...
      return (
        (includeDomains.length === 0 || includeDomains.includes(domain)) &&
    ...
    const generalResults = sortedResults
